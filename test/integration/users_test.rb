...
>>>>>>> 62222906787e3229a276ab1acce704eccd16229a
    ...
    test 'Should_see_sign_up_not_logged_in' do
        click_on('Sign out')
        assert page.has_field?('btn-primary')

        # click_on('Sign out')
        # within('div#globalNavbar.collapse.navbar-collapse') do
        #     assert page.has_field?('ul.nav.navbar-nav.navbar-right')
        # end
    end
    ...
    #     within('div#heroImage.row') do
    #         assert_text('JOIN WEEKLY LIVESTREAM DISCUSSIONS RIGHT HERE')
    #         #assert has_field?('heroTitle.row')
    #         page.find('#heroImage').first('.heroTitle')[:id]
    #         #assert page.has_field?('h1#heroTitle')
    #     end
    # end
